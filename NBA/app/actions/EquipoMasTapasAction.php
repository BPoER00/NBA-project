    ...
    public function execute()
    {
        ...
        foreach($query as $item)
        {
            ...
            foreach($equipo_jugador as $item2)
            {
                ...
                $suma += $estadisticas_equipo_jugador->sum('bloqueos');
                $punteo[$contador]['bloqueos'] = $suma;
            }
        ...
        for($i = 0; $i < 30; $i++)
        {
            for($o = 0; $o < 30; $o++)
            {
                if($punteo[$i]['bloqueos'] < $punteo[$o]['bloqueos'])
                {
                    $orden++;
                }
            }
            $ordenado[$orden]['nombre'] = $punteo[$i]['nombre'];
            $ordenado[$orden]['bloqueos'] = $punteo[$i]['bloqueos'];
